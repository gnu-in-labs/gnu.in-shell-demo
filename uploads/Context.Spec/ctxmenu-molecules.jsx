...
public:
  ...
  flutter: `class MenuSection extends StatelessWidget {
  const MenuSection({super.key, required this.label,
                     required this.rows});
  final String label; final List<RowSpec> rows;
  @override Widget build(BuildContext c) => Column(
    crossAxisAlignment: CrossAxisAlignment.stretch,
    children: [
      SectionLabel(label),
      ...rows.map((r) => MenuRow.from(r)),
    ],
  );
}`,
};
...
v->setStyleSheet("background: rgba(0,0,0,.22); color: white;"
  ...
  flutter: `class BrandedHeader extends StatelessWidget {
  @override Widget build(BuildContext c) => Stack(children: [
    Container(
      height: 36, padding: const EdgeInsets.symmetric(horizontal: 12),
      color: GnuTheme.accent,
      child: Row(children: const [
        SysterGlyph(size: 20, state: SysterState.listening),
        SizedBox(width: 8),
        Expanded(child: Column(
          crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('Gnu.In-Shell', style: TextStyle(color: Colors.white,
            fontSize: 11, fontWeight: FontWeight.bold)),
          Text('DESKTOP · SLOT 0', style: TextStyle(color: Colors.white,
            fontFamily: 'IBMPlexMono', fontSize: 9, letterSpacing: .8)),
        ])),
        VersionChip(text: 'v6.2'),
      ]),
    ),
    Positioned.fill(child: IgnorePointer(child: Opacity(
      opacity: .18, child: Image.asset('dither.png', repeat: ImageRepeat.repeat)))),
  ]);
}`,
};
...
Rectangle {
  ...
  */
  ...
  flutter: `class IdentityHeader extends StatelessWidget {
  const IdentityHeader({super.key, required this.target});
  final WidgetTarget target;
  @override Widget build(BuildContext c) => Container(
    padding: const EdgeInsets.all(12),
    color: GnuTheme.accent.withOpacity(.04),
    child: Column(children: [
      Row(children: [
        WidgetThumb(icon: target.icon, size: 36),
        const SizedBox(width: 8),
        Expanded(child: Column(
          crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(target.name, style: const TextStyle(
              fontSize: 12, fontWeight: FontWeight.w600)),
            Text('\${target.w} × \${target.h} · ws \${target.ws}',
              style: TextStyle(fontFamily: 'IBMPlexMono',
                fontSize: 10, color: GnuTheme.fgDim)),
        ])),
        IdPill(text: target.id),
      ]),
...
protected:
  ...
  flutter: `class TileGrid extends StatelessWidget {
  ...
  @override Widget build(BuildContext c) => GridView.count(
    ...
    children: tiles.map((t) => TileCell(
      spec: t, onTap: () => onPick(t.k),
    )).toList(),
  );
}`,
};
...
Item {
  ...
  flutter: `class RadialWheel extends StatefulWidget { /* ... */ }
class _RadialPainter extends CustomPainter {
  _RadialPainter(this.items, this.hoverIdx);
  final List<RadialItem> items; final int hoverIdx;
  @override void paint(Canvas c, Size s) {
    final cx = s.width/2, cy = s.height/2; final R = 90.0, r = 32.0;
    final n = items.length;
    for (var i = 0; i < n; ++i) {
      final a0 = i/n * 2*math.pi - math.pi/2 - math.pi/n;
      final a1 = a0 + 2*math.pi/n;
      final path = Path()
        ..moveTo(cx + math.cos(a0)*r, cy + math.sin(a0)*r)
        ...
        ..arcTo(Rect.fromCircle(center: Offset(cx,cy), radius: r),
                a1, -(a1-a0), false)
        ..close();
      c.drawPath(path, Paint()..color = i == hoverIdx
        ? GnuTheme.accent : GnuTheme.surface);
    }
  }
  @override bool shouldRepaint(_RadialPainter o) => o.hoverIdx != hoverIdx;
}`,
};
...
public:
  ...
  flutter: `class PillRow extends StatefulWidget {
  const PillRow({super.key, required this.icon, required this.label,
                 this.danger = false, this.indexInStack = 0});
  // ...
  @override State<PillRow> createState() => _PillRowState();
}
class _PillRowState extends State<PillRow> {
  ...
  @override Widget build(BuildContext c) => MouseRegion(
    ...
    child: AnimatedContainer(
      duration: Duration(milliseconds: 140 + widget.indexInStack*8),
      curve: Curves.easeOutCubic,
      width: 180, height: 36,
      padding: const EdgeInsets.symmetric(horizontal: 14),
      transform: Matrix4.translationValues(h ? 6 : 0, 0, 0),
      decoration: BoxDecoration(
        color: h ? (widget.danger ? const Color(0xFFFF5040)
                                   : GnuTheme.accent)
                 : GnuTheme.surface,
        borderRadius: BorderRadius.circular(18),
        boxShadow: [BoxShadow(
          offset: const Offset(0, 4), blurRadius: 12,
          color: Colors.black.withOpacity(h ? .18 : .08))],
      ),
      child: Row(children: [
        Icon(widget.icon, size: 14,
          color: h ? Colors.white : GnuTheme.fg1.withOpacity(.7)),
...
Item {
  ...
  */
  ...
  flutter: `class BubbleCluster extends StatefulWidget { /* ... */ }
class _BubbleClusterState extends State<BubbleCluster>
    with TickerProviderStateMixin {
  late final List<AnimationController> _ctrls;
  // one controller per bubble, staggered by 36ms (M.12 motion)
  @override void initState() {
    super.initState();
    _ctrls = List.generate(widget.nodes.length, (i) =>
  ...
  @override Widget build(BuildContext c) => Stack(children: [
    CustomPaint(painter: _ConnectorPainter(/*...*/)),
    for (var i = 0; i < widget.nodes.length; ++i)
      AnimatedBuilder(animation: _ctrls[i],
        builder: (_, __) => Transform.scale(
          scale: Curves.elasticOut.transform(_ctrls[i].value),
          child: BubbleNode(node: widget.nodes[i]))),
    BubbleHub(),  // mascot center
  ]);
}`,
};
...
public:
  ...
  flutter: `class SubCascade extends StatefulWidget {
  /* root MenuShell + Overlay-based sub MenuShell.
     Sub uses ClipPath(clipper: _CircleReveal(t))
     + AnimationController(280ms) curve: Curves.easeOutCubic.
     Root applies Transform.translate(x: -2 * t) when sub is open. */
}`,
};

...
public:
  ...
  flutter: `class AccountRow extends StatelessWidget {
  const AccountRow({super.key, required this.account});
  final Account account;
  @override Widget build(BuildContext c) => Container(
    height: 44, padding: const EdgeInsets.symmetric(horizontal: 12),
    decoration: BoxDecoration(color: GnuTheme.hover,
      borderRadius: BorderRadius.circular(8)),
    child: Row(children: [
      GnuAvatar(size: 32, initials: account.initials,
                status: account.status),
      ...
      Expanded(child: Column(
        ...
        Text.rich(TextSpan(children: [
          TextSpan(text: account.name, style: const TextStyle(
            fontSize: 13, fontWeight: FontWeight.w600,
            letterSpacing: -0.01)),
          TextSpan(text: ' @\${account.handle}',
            style: const TextStyle(fontSize: 13,
              fontWeight: FontWeight.w500, color: GnuTheme.accent)),
        ])),
        Text('\${account.status} · \${account.domain}',
          style: TextStyle(fontFamily: 'IBMPlexMono', fontSize: 10.5,
            color: GnuTheme.fgDim)),
      ])),
      const Text('▸', style: TextStyle(color: GnuTheme.accent,
...
public:
  ...
  flutter: `enum ToastTone { info, success, danger }
class GnuToast extends StatelessWidget {
  const GnuToast({super.key, required this.tone,
                  required this.title, required this.body,
                  required this.age, this.onClose});
  final ToastTone tone; final String title, body, age;
  final VoidCallback? onClose;
  Color get _toneColor => switch (tone) {
    ToastTone.info    => GnuTheme.accent,
    ToastTone.success => const Color(0xFF5F7F52),
    ToastTone.danger  => const Color(0xFFE63A1F),
  };
  @override Widget build(BuildContext c) => Container(
    ...
    decoration: BoxDecoration(
      ...
      boxShadow: const [BoxShadow(offset: Offset(0, 6),
        blurRadius: 20, color: Color(0x1F111418))],
    ),
    child: Row(crossAxisAlignment: CrossAxisAlignment.start,
               children: [
      Container(width: 22, height: 22,
        decoration: BoxDecoration(color: _toneColor,
          shape: BoxShape.circle),
        alignment: Alignment.center,
        child: Icon(_toneIcon(), color: Colors.white, size: 12)),
      const SizedBox(width: 10),
      Expanded(child: Column(/* title + body */)),
      Text(age, style: TextStyle(fontFamily: 'IBMPlexMono',
...
public:
  ...
  flutter: `enum EmptyVariant { empty, loading, error, whispered }
class GnuEmpty extends StatelessWidget {
  const GnuEmpty({super.key, required this.title, required this.body,
                  required this.icon, this.primary, this.secondary,
                  this.variant = EmptyVariant.empty});
  /* full impl : DottedBorder around Container,
     Column with circular illustration, title, body, button row, tag */
  @override Widget build(BuildContext c) => DottedBorder(
    color: GnuTheme.border, strokeWidth: .5,
    dashPattern: const [3, 3], borderType: BorderType.RRect,
    ...
    child: Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        SizedBox(width: 220, child: Text(body, textAlign: TextAlign.center,
          style: TextStyle(fontFamily: 'IBMPlexMono', fontSize: 11,
            color: GnuTheme.fgDim))),
        const SizedBox(height: 14),
        Row(mainAxisSize: MainAxisSize.min, children: [
          if (primary != null)   _CtaButton.primary(primary!),
          if (secondary != null) _CtaButton.secondary(secondary!),
        ]),
        const SizedBox(height: 6),
        GnuTag(variant.label, variant: TagVariant.outline),
      ]),
    ),
  );
}`,
...
public:
  ...
  flutter: `class GnuBreadcrumb extends StatelessWidget {
  const GnuBreadcrumb({super.key, required this.trail,
                       required this.onBack, required this.onClose,
                       required this.onJump});
  final List<String> trail; final VoidCallback onBack, onClose;
  ...
  @override Widget build(BuildContext c) => Container(
    ...
    child: Row(children: [
      ...
      for (var i = 0; i < trail.length; ++i) ...[
        InkWell(onTap: () => onJump(i),
          child: Text(
            i == trail.length - 1
              ? trail[i].toUpperCase() : trail[i],
            style: i == trail.length - 1
              ? const TextStyle(color: GnuTheme.accent,
        ...
        if (i < trail.length - 1) const Padding(
          padding: EdgeInsets.symmetric(horizontal: 6),
          child: Text('/', style: TextStyle(color: Colors.black38)),
        ),
      ],
      const Spacer(),
      InkWell(onTap: onClose, child: const Text('⎋')),
    ]),
  );
}`,
};

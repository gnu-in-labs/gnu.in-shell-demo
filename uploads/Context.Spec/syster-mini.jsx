  ...
  return (
    <svg ref={ref} width={size} height={size} viewBox="0 0 256 256" style={{ flexShrink: 0, overflow: 'visible' }} aria-label={`syster mini ${state}`}>
      ...
      <g>
        ...
        {/* terminal screen */}
        <rect x="62" y="100" width="120" height="116" rx="12" fill={MINI.screen} opacity={sleeping ? 0.82 : 1} />

        {/* prompt — chevron + cursor. In transmit the cursor becomes animated "..." dots. */}
        <g opacity={sleeping ? 0.32 : 1}>
          ...
          {state === 'transmit' ? (
            ...
            <line x1="118" y1="190" x2="148" y2="190" stroke={MINI.prompt} strokeWidth="10" strokeLinecap="round">
              {play && !sleeping && <animate attributeName="opacity" values="1;1;0;0;1" dur="1.6s" repeatCount="indefinite" keyTimes="0;0.45;0.5;0.95;1" />}
            </line>
          )}
        </g>
        ...
        {/* ── STATE: veille — sleep z's drift up off the shell ── */}
        {state === 'veille' && (
          <g fill={MINI.prompt} fontFamily="ui-monospace, monospace" fontWeight="700">
            <text x="172" y="76" fontSize="16">
              z
              {play && <>
                <animate attributeName="opacity" values="0;0.8;0" dur="2.8s" repeatCount="indefinite" />
                <animateTransform attributeName="transform" type="translate" values="0 0;6 -16" dur="2.8s" repeatCount="indefinite" />
              </>}
            </text>
            <text x="186" y="58" fontSize="24">
              Z
              {play && <>
                <animate attributeName="opacity" values="0;0.9;0" dur="2.8s" begin="0.6s" repeatCount="indefinite" />
                <animateTransform attributeName="transform" type="translate" values="0 0;8 -20" dur="2.8s" begin="0.6s" repeatCount="indefinite" />
              </>}
            </text>
          </g>
        )}

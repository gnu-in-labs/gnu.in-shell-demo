...
chunk_version: 1
project: gnuin
supersedes: "[[rttc-v0.4.0-drift-note]]"
tags:
...
### 2026-06-08 · 4.0-P5 · READINESS + RENUMBERING 0.4.0
...
- **Renumbering** — la spec passe de **v3.1 → v0.4.0** pour suivre le semver du
...
- **Livré** — `blob.in/gen/` : `GnuTheme.qml` + `gnu_theme.h` + `gnu_theme.dart`
  réellement générés depuis tokens.json (les 3 cibles couleur du générateur).
...
- **Livré** — `blob.in/blobin-gen/` : générateur réel (serde) qui lit
  tokens.json et émet : `--rust` (blobin_tokens.rs · consts du solveur),
  `--qml` (GnuTheme singleton), `--header` (gnu_theme.h constexpr),
  `--dart` (GnuTheme const class). Tests unitaires (camelCase, ARGB).
...
C'est ce qui ferme le drift documenté dans `rttc-v0.4.0-drift-note.md` :

```
tokens.json   (source unique — déjà vraie pour GnuTheme QML/Qt/Flutter)
     │  build-time codegen
     ├──→ gnu_theme.qml / .h / .dart      (couleurs, surfaces — existant)
     └──→ blobin_tokens.rs            (durées, courbes — NOUVEAU 4.0)
              │
              ▼

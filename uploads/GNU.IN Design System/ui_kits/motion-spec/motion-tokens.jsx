...
// Mapping : neutral → CSS / QML (Quickshell) / Qt C++ / GPUI (Rust).
...
// gpui column: easing closure form — pass to cx.animate(dur, |t| gpui_ease(t), …)
// bezier(p1x,p1y,p2x,p2y) is a helper you define once: |t| cubic_bezier(p1x,p1y,p2x,p2y,t)
const MEASE = {
  standard:  { bz: [0.20, 0.70, 0.20, 1.00], qt: 'OutCubic',    gpui: 'bezier(.20,.70,.20,1)',   label: 'standard' },
  decel:     { bz: [0.16, 1.00, 0.30, 1.00], qt: 'OutExpo',     gpui: 'bezier(.16,1,.30,1)',     label: 'décel · entrée' },
  accel:     { bz: [0.70, 0.00, 0.84, 0.00], qt: 'InExpo',      gpui: 'bezier(.70,0,.84,0)',     label: 'accel · sortie' },
  snap:      { bz: [0.40, 0.00, 0.20, 1.00], qt: 'InOutCubic',  gpui: 'bezier(.40,0,.20,1)',     label: 'snap · commit' },
  gentle:    { bz: [0.32, 0.72, 0.32, 1.00], qt: 'OutQuad',     gpui: 'bezier(.32,.72,.32,1)',   label: 'doux · hover' },
  overshoot: { bz: [0.34, 1.36, 0.64, 1.00], qt: 'OutBack',     gpui: 'bezier(.34,1.36,.64,1)', label: 'overshoot · atomic' },
  elastic:   { bz: [0.50, 1.55, 0.40, 1.00], qt: 'OutElastic',  gpui: 'bezier(.50,1.55,.40,1)', label: 'élastique · bloom' },
  linear:    { bz: [0.00, 0.00, 1.00, 1.00], qt: 'Linear',      gpui: 'linear',                  label: 'linéaire · pulse' },
};
...
function snipGpui(s) {
  const e = MEASE[s.ease] || MEASE.standard;
  const delay = s.delay ? `\n  // stagger delay: ${s.delay}ms` : '';
  return `// GPUI · ${s.id || 'animation'}
// cx.animate(
//   Duration::from_millis(${s.dur || 'MDUR_BASE'}),
//   |t| bezier_ease(${e.bz.join(', ')}, t),
//   |view, v, cx| { /* map v 0..1 to property */${delay} cx.notify(); });
// easing gpui: ${e.gpui}  qt: QEasingCurve::${e.qt}`;
}
...
Object.assign(window, {
  ...
  snipCSS, snipQML, snipQt, snipGpui, snipNeutral, snipAll,
});

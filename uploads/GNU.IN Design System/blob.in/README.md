...
gen/                     generated theme files (gnu_theme.h · gnu_theme.gpui.rs; GnuTheme.qml opt-in)
...
`blobin-gen` lowers it to each engine — **Rust/C++ first**; Flutter removed;
QML demoted to opt-in:

```
tokens.json ──blobin-gen──┬──> blobin-core/src/tokens.rs   (Rust solver consts)   PRIMARY
                          ├──> gen/gnu_theme.h               (C++ constexpr)        PRIMARY
                          └──> gen/gnu_theme.gpui.rs         (GPUI Rgba + motion)   PRIMARY
                          ··> [--qml gen/GnuTheme.qml]       (Qt/Quickshell opt-in)

just tokens        # regenerate primary targets (Rust · C++ · GPUI)
just tokens-qml    # opt-in: regenerate the QML singleton for Qt surfaces
...
| Bridge | `cxx` brut on the hot path · `cxx-qt` on the reactive surface (QML opt-in) |

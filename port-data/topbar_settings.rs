// gnuin-compose-core :: topbar + widget settings (port target)
// Mirrors ShellSettingsService's `topbar*` + `widgetEnabled` surface
...
// Central's Control Center Topbar/Widgets groups and the live bar consume the
// same fields by name.
...
pub fn resolve_bar(s: &TopbarSettings, cx: BarContext) -> ResolvedBar {
...
mod topbar_tests {
    ...
    fn disabling_a_widget_drops_it_from_its_slot() {
        let mut s = TopbarSettings::default();
